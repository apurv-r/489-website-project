...
export default function Confirmation() {
  ...
  const hostName =
    ...
    'Host';
  const referenceId = stateConfirmation?.referenceId || `NXA-${new Date().getFullYear()}-${String(Date.now()).slice(-5)}`;
        ...
        {loading && (
          ...
          <div className="conf-card">
            ...
            <div className="conf-listing-row">
              ...
              <div>
                ...
                <div className="conf-listing-addr"><i className="bi bi-geo-alt-fill me-1"></i>{address}</div>
              </div>
            </div>
          </div>
          ...
          <div className="conf-card">
            ...
            <div className="conf-detail-rows">
              <div className="conf-detail-row"><span className="conf-detail-label">Check-in</span><span className="conf-detail-value">{formatDateLabel(checkInDate)}</span></div>
              <div className="conf-divider"></div>
              <div className="conf-detail-row"><span className="conf-detail-label">Check-out</span><span className="conf-detail-value">{formatDateLabel(checkOutDate)}</span></div>
              <div className="conf-divider"></div>
              <div className="conf-detail-row"><span className="conf-detail-label">Duration</span><span className="conf-detail-value">{days} day{days === 1 ? '' : 's'}</span></div>
              <div className="conf-divider"></div>
              <div className="conf-detail-row"><span className="conf-detail-label">${dailyRate.toFixed(2)} × {days} day{days === 1 ? '' : 's'}</span><span className="conf-detail-value">${subtotal.toFixed(2)}</span></div>
              <div className="conf-detail-row"><span className="conf-detail-label">Service fee</span><span className="conf-detail-value">${serviceFee.toFixed(2)}</span></div>
              <div className="conf-divider"></div>
              <div className="conf-detail-row conf-total-row"><span>Total paid</span><span>${total.toFixed(2)}</span></div>
            </div>
          </div>
          ...
          <div className="conf-card">
            ...
            <div className="conf-host-row">
              ...
              <div>
                ...
                <div className="conf-host-stat"><i className="bi bi-star-fill me-1"></i>{rating} · {reviewCount} reviews · Verified Host</div>
              </div>
          ...
          <div className="conf-card">
            ...
            <p className="conf-getting-addr"><i className="bi bi-geo-alt-fill me-2"></i>{address}</p>
