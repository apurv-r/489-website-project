...
export default function HostDashboard() {
  ...
  const avgRating = ratingsWithReviews.length > 0
    ? (ratingsWithReviews.reduce((sum, l) => sum + l.ratingAverage, 0) / ratingsWithReviews.length).toFixed(1)
    : null;
        ...
        <main className="dash-main">
          ...
          <div className="dash-stat-grid">
            ...
            <div className="dash-stat-card">
              ...
              <div className="dash-stat-body">
                <div className="dash-stat-value">{loading ? '—' : `$${thisMonthEarnings.toFixed(0)}`}</div>
            ...
            <div className="dash-card">
              ...
              <div className="dash-booking-list">
                {loading ? (
                  ...
                  <p style={{ color: 'var(--nexa-gray-500)', padding: '1rem 0' }}>No bookings yet.</p>
                  ...
                  return (
                    <Link to={`/host/booking-details?id=${b._id}`} className="dash-booking-item" key={b._id}>
                      {img
                        ...
                        : <div className="dash-booking-thumb" style={{ background: 'var(--nexa-surface-2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><i className="bi bi-image" style={{ color: 'var(--nexa-gray-500)' }} /></div>
                      }
                      <div className="dash-booking-info">
                        <div className="dash-booking-name">{guestName} — {listingTitle}</div>
              ...
              <div className="dash-card">
                ...
                <div className="lsr-listing-mini-list">
                  {loading ? (
                    ...
                    <p style={{ color: 'var(--nexa-gray-500)', padding: '0.5rem 0' }}>No listings yet.</p>
                  ) : listings.slice(0, 3).map(l => (
                    <Link to={`/host/listing-details?id=${l._id}`} className="lsr-listing-mini" key={l._id}>
                      {l.imageUrls?.[0]
                        ? <img src={l.imageUrls[0]} alt="" className="lsr-listing-mini-img" />
                        : <div className="lsr-listing-mini-img" style={{ background: 'var(--nexa-surface-2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><i className="bi bi-image" style={{ color: 'var(--nexa-gray-500)' }} /></div>
                      }
                      <div className="lsr-listing-mini-info">
                        <div className="lsr-listing-mini-name">{l.title}</div>
                        <div className="lsr-listing-mini-meta">
                          ${l.dailyRate}/day · <span className={`lsr-badge ${l.isPublished ? 'lsr-badge-active' : 'lsr-badge-pending'}`}>{l.isPublished ? 'Active' : 'Pending'}</span>
                        </div>
                      </div>
                    </Link>
                  ))}
